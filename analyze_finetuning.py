...
# Load the JSON file
file_path = "results_finetuning.json"  # Replace with the correct json file
...
print("---------------- Reporting Matching Degree (only Exact-Match)")
for label, metrics in BUCKET_METRICS_BY_LABEL.items():
    predicted = metrics["PREDICTED_COUNT"]
    ground_truth = metrics["GROUND_TRUTH_COUNT"]
    if predicted > 0:
        ratio = predicted / ground_truth
        print(f"{label}: {ratio} (Predicted / Ground Truth)")